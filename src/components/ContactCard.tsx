...
const ContactCard:React.FC<ContactCardProps> = ({ region, address, countrycode, phonenumber, id, lat, lng, zoom }) => {
  return (
    <s.Contact className="card-wrapper">
      ...
      <s.Contact className="card-container">
        <s.Contact className="card-box-left">
          <s.hr className="card-line" />
          <s.Contact className="contact-info-box">
            <s.StyledP className="contact-info">
              {address}
            </s.StyledP>
            <s.Atag href={`tel:${phonenumber}`}>{countrycode}+ {phonenumber}</s.Atag>
          </s.Contact>
        </s.Contact>
