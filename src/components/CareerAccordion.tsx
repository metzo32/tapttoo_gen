...
const Accordion = () => {
  ...
  const handleToggle = (index: number) => {
    ...
    setOpenStates(
      (
        prevStates // setOpenStates로 openStates 배열 업데이트
      ) => prevStates.map((state, i) => (i === index ? !state : state)) //map으로 배열을 순회하며 클릭된 항목 상태 바꿈
    );
  };

  return (
    <s.Accordion className="accordion-wrapper">
      {CareerAccordionData.map(
        (
          data,
          index // 매개변수 data와 index를 받아 순회
        ) => (
          <s.Accordion
            key={data.id}
            className={openStates[index] ? "acc-open" : "acc"}
          >
            ...
            />
            <s.Accordion
              className={openStates[index] ? "cardbox-open" : "cardbox"}
            >
