...
export default function SearchEventProfile() {
  ...
  const positions = useRef(
    eventData.map(() => ({  // 각 요소의 목표 위치, 현재 위치 저장
      target: { x: 0, y: 0 }, 
      current: { x: 0, y: 0 },
    }))
  );

  useEffect(() => { //클린업. 언마운트 될 때 requestAnimationFrame으로 예약된 애니메이션이 있다면 모두 취소
    return () => {
  ...
  const handleMouseLeave = (index: number) => () => {
    positions.current[index].target = { x: 0, y: 0 };
    startAnimation(index);
  };
  ...
  const startAnimation = (index: number) => {
    ...
    const animate = () => {
      ...
      if (imgRefs.current[index]) {
        imgRefs.current[index]!.style.transform = `translate(${pos.current.x}px, ${pos.current.y}px)`;
      }

      if (Math.abs(pos.target.x - pos.current.x) > 0.1 || Math.abs(pos.target.y - pos.current.y) > 0.1) {
  ...
  return (
    <s.Search className="top-container column">
      <s.StyledH3 className="search-title upcoming">UPCOMING EVENTS</s.StyledH3>
